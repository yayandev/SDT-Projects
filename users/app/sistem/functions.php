...
function send_requestCategory($data)
{
  ...
  $existCategory = mysqli_query($conn, "SELECT * FROM kategori WHERE kategori = '$category' ");
  $requestSame = mysqli_query($conn, "SELECT * FROM $tablename WHERE category = '$category' AND username = '$username'");
